...
\begin{enumerate}
    ...
    \item \textbf{Rational:} The real number $r$ is rational if there exists integers $p$ and $q$ with $q \neq 0$ such that $r = \frac{p}{q}$.
...
\section{Laws \& Axioms}
\begin{itemize}
    \item The sets $\mathbb{Z}$ and $\mathbb{R}$ are closed under addition and multiplication. That is, for any $a, b \in \mathbb{Z}$ (respectively, $a, b \in \mathbb{R}$), both $a + b \in \mathbb{Z}$ (respectively, $\in \mathbb{R}$) and $a \cdot b \in \mathbb{Z}$ (respectively, $\in \mathbb{R}$).
    \item For integers $x$ and $n$ with $n > 0$, $x^n$ is even if and only if $x$ is even, and $x^n$ is odd if and only if $x$ is odd.
    
\end{itemize}


\newpage
\input{Tables/Logical Equivalences}
\newpage
\input{Tables/Rules of Inference}
\input{Tables/Quantified Rules of Inference and Equivalences}
